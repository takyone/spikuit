...
すでに優れたツールがあります。Spikuit はそれらの交差領域を埋めます:

| ツール | 強み | Spikuit が加えるもの |
|--------|------|---------------------|
| Anki | 最高水準のスケジューリング | 概念間の関係 + グラフベースの検索 |
| Obsidian | 豊かな知識リンク | 間隔反復 + 適応的な重み付け |
| DeepTutor | 文脈を考慮した学習支援 | FSRSによる長期記憶の定着 |

Spikuit はこれらのアイデアを組み合わせます: **学習 → 定着 → 検索** を一つの流れで。
