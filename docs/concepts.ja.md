...
**Brain**は自己完結型のナレッジ空間 ——
[Obsidian vault](https://obsidian.md/)やgitリポジトリのようなもの。
...
my-project/
└── .spikuit/
    ├── config.toml    # Brain設定（名前、埋め込み）
    ├── circuit.db     # SQLiteデータベース（ニューロン、シナプス、FSRS）
    └── cache/         # 埋め込みキャッシュ
```

### 複数Brain

Brainはいくつでも持てる — プロジェクト、ドメイン、トピックごとに1つ。
各Brainは独立: 独自のニューロン、シナプス、復習履歴。
...
gitと同様に、`spkt`はカレントディレクトリから上に辿って`.spikuit/`を探す。
Brainディレクトリ（またはそのサブディレクトリ）内でコマンドを実行すればそのまま動く。
別のBrainを操作するには`--brain`を使う:
...
- **TutorSession**: 1対1チュータリング — スキャフォールド型指導、ヒント段階開示、ギャップ検出、誤答解説

### Quiz（セッションが使う評価ツール）

- **Flashcard**（core）: セルフグレード、LLM不要
- **AutoQuiz**: LLM生成問題、プログラム的採点
